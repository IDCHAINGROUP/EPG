...
*Setup instructions for a gitian build of encocoinplus using a VM or physical system.*

Gitian is the deterministic build process that is used to build the encocoinplus
...
to the encocoinplus GitHub release page.
...
- [Building encocoinplus](#building-encocoinplus-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/encocoinplus-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/encocoinplus-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/encocoinplus-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/encocoinplus-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/encocoinplus-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/encocoinplus-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/encocoinplus-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@encocoin.net) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/encocoinplus-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/encocoinplus-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/encocoinplus-osx-*-build.assert
```
