#!/bin/bash
# Copyright (c) 2013-2022 The Veles Core developers
...
TEMP_PATH=$(mktemp -d)
USER='encocoinplus'
CONFIG_FILENAME='encocoinplus.conf'
DATADIR_PATH='/home/encocoinplus/.encocoinplus'
COIN_DAEMON='encocoinplusd'
COIN_CLI='encocoinplus-cli'
INSTALL_PATH='/usr/local/bin'
COIN_TGZ_URL='https://github.com/Encocoin/encocoinplus/releases/download/v1.0.0/encocoinplus-1.0.0-linux.tar.gz'
COIN_NAME='EncocoinPlus'
COIN_NAME_SHORT='encocoinplus'
COIN_PORT=29000
RPC_PORT=29001
START_STOP_TIMEOUT=20
START_STOP_RETRY_TIMEOUT=5
KEY_GEN_TIMEOUT=20
ARCHIVE_PATH=''

# Autodetection
NODEIP=$(wget --no-check-certificate -qO - api.ipify.org)   #NODEIP=$(curl -s api.ipify.org)
NEED_REINDEX=""
...
function check_installation() {
  ...
  if [ -e "${INSTALL_PATH}/${COIN_DAEMON}" ] && [ -e "/etc/systemd/system/${COIN_NAME_SHORT}.service" ]; then
    echo "yes"
    start_update
  else
    echo "no"
...
function download_and_copy() {
  ...
  wget --no-check-certificate -q $COIN_TGZ_URL || perr "Failed to download installation archive"
  ...
  cp "${TEMP_PATH}/${ARCHIVE_PATH}${COIN_DAEMON}" "${INSTALL_PATH}/${COIN_DAEMON}" || "Failed to copy ${COIN_DAEMON} to ${INSTALL_PATH}"
  cp "${TEMP_PATH}/${ARCHIVE_PATH}${COIN_CLI}" "${INSTALL_PATH}/${COIN_CLI}" || "Failed to copy ${COIN_CLI} to ${INSTALL_PATH}"
...
## Generated by EncocoinPlus script masternode.sh ${SCRIPT_VERSION}
[Unit]
...
function stop_service() {
  ...
  ${INSTALL_PATH}/${COIN_CLI} -datadir=${DATADIR_PATH} stop >/dev/null 2>&1
  tries=0
...
port=$COIN_PORT
# Official nodes
addnode=149.28.235.51
addnode=108.61.23.94
addnode=207.246.82.120
addnode=207.148.17.239
addnode=45.76.6.98
EOF
  chown "$USER:$USER" "${DATADIR_PATH}" -R
}

function create_key() {
  if ! [ $ARG1 ] || ! [ $ARG1 == '--nonint' ]; then # skip reading in non-interactive mode
    echo -e "\nEnter your ${RED}${COIN_NAME_SHORT} Masternode Private Key${NC}. Leave it blank to generate a new ${RED}Masternode Private Key${NC} for you:"
    read -e COINKEY
  fi
  if [[ -z "$COINKEY" ]]; then
    echo -en "${ST}   Generating masternode private key, please wait a moment ...         "
    ${INSTALL_PATH}/$COIN_DAEMON -daemon -datadir=${DATADIR_PATH} >/dev/null 2>&1
    sleep ${KEY_GEN_TIMEOUT}
    if [ -z "$(ps axo cmd:100 | grep $COIN_DAEMON)" ]; then
      perr "${RED}${COIN_NAME_SHORT} server couldn not start. Check /var/log/syslog for errors.${NC}"
    fi
    COINKEY=$(${INSTALL_PATH}/${COIN_CLI} -datadir=${DATADIR_PATH} createmasternodekey)
    if [ "$?" -gt "0" ];then
...
function get_ip() {
  ...
  if [[ -n $HAS_DIG ]]; then
    ...
    ifaces=$(ip -o link | grep -v 'DOWN' | awk '!/lo/ {gsub( /\:/, ""); print $2}')
  ...
  if [[ -n $ifaces ]]; then
    if [[ -n $HAS_CURL ]]; then
      ...
      NODE_IPS+=($(wget --no-check-certificate -qO - api.ipify.org))
      [ $(wc -w <<< $ifaces) -gt 1 ] && pwarn "More than one network interface detected, but ${YELLOW}curl${NC} command required 
for multiple IP detection is not installed, using the default one: ${NODE_IPS[0]}"
    fi
  else
...
external IP address"
  ...
  NODEIP=$(wget --no-check-certificate -qO - api.ipify.org)
...
function print_logo() {
  echo -e "\n${BGREEN}EncocoinPlus Masternode Installator${NC}"
}
...
function start_installation() {
  ...
  echo -e "\n${BGREEN}Congratulations, ${COIN_NAME} has been installed successfuly.${NC}\n"
}
