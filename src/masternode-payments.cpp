...
bool IsBlockValueValid(const CBlock& block, CAmount nExpectedValue, CAmount nMinted)
{
    ...
    LogPrint(BCLog::MASTERNODE,"IsBlockValueValid() : nMinted: %d, nExpectedValue: %d\n", FormatMoney(nMinted), FormatMoney(nExpectedValue));

    if (sporkManager.IsSporkActive(SPORK_90_BLOCK_REWARD_ENFORCEMENT)) {
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake)
{
    ...
    } /*else {  // No dev reward when MN payee is missing
