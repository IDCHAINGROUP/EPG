...
#include "qt/encocoinplus/forms/ui_snackbar.h"
#include "qt/encocoinplus/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(encocoinplusGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &encocoinplusGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
