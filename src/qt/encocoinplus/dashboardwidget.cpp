...
#include "qt/encocoinplus/dashboardwidget.h"
#include "qt/encocoinplus/forms/ui_dashboardwidget.h"
#include "qt/encocoinplus/sendconfirmdialog.h"
#include "qt/encocoinplus/txrow.h"
...
DashboardWidget::DashboardWidget(encocoinplusGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareEPG, "square-chart-encocoinplus");
    setCssProperty(ui->labelEPG, "text-chart-encocoinplus");
    ...
    setCssProperty(ui->labelAmountEPG, "text-stake-encocoinplus-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &encocoinplusGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &encocoinplusGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &encocoinplusGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        qreal encocoinplus = 0;
        if (chartData->amountsByCache.contains(num)) {
            std::pair <qint64, qint64> pair = chartData->amountsByCache[num];
            encocoinplus = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalEPG += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesEPG.append(encocoinplus);

        int max = std::max(encocoinplus, encocoinplus);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(69,79,131));
    ...
    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalEPG > 0) {
        setCssProperty(ui->labelAmountEPG, "text-stake-encocoinplus");
    } else {
        setCssProperty(ui->labelAmountEPG, "text-stake-encocoinplus-disable");
    }
    forceUpdateStyle({ui->labelAmountEPG});
