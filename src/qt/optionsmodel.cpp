...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    ...
    if (settings.contains("nAnonymizeencocoinplusAmount") || reset)
        SoftSetArg("-anonymizeencocoinplusamount", settings.value("nAnonymizeencocoinplusAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in encocoinplus.cpp during shotdown to also remove the window positions
