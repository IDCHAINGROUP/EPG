...
#if defined(NDEBUG)
#error "encocoinplus cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uencocoinplus) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateEPGSupply(int nHeightStart)
{
	...
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			...
			uiInterface.ShowProgress(_("Recalculating EPG supply..."), percent);
		}
...
bool AcceptableInputs(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool isDSTX)
{
...

    {
        ...
        if (isDSTX) {
            mempool.PrioritiseTransaction(hash, hash.ToString(), 1000, 0.1);
...
CAmount GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) {
        nSubsidy = 200000 * COIN;   // 0.5%
    } else if (nHeight <= 2000) { 
        nSubsidy = 5000 * COIN;
    } else if (nHeight <= 20000) {
        nSubsidy = 1000.05 * COIN;
    } else if (nHeight <= 50000) {
        nSubsidy = 2000.05 * COIN;
    } else if (nHeight <= 80000) {
        nSubsidy = 2500.05 * COIN;
    } else if (nHeight <= 90000) {
        nSubsidy = 1200.05 * COIN;
    } else if (nHeight <= 100000) {
        nSubsidy = 600.05 * COIN;
    } else if (nHeight <= 110000) {
        nSubsidy = 300.05 * COIN;
    } else if (nHeight <= 120000) {
        nSubsidy = 150.05 * COIN;
    } else if (nHeight <= 130000) {
        nSubsidy = 75.05 * COIN;
    } else {
        nSubsidy = 500.05 * COIN;
    }
...
{
    CAmount nDevPayment = 0;

    if (nHeight <= 2000) { 
        nDevPayment = 0;
    } else if (nHeight <= 20000) {
        nDevPayment = 500.000000001 * COIN;    // 0.1 satoshi here does not affect value
    } else if (nHeight <= 50000) {             // but prevents rounding issues that could
        nDevPayment = 1000.000000001 * COIN;   // occur in some special cases.
    } else if (nHeight <= 55500) {
        nDevPayment = 1300.000000001 * COIN;
    } else if (nHeight <= 80000) {
        nDevPayment = 1200.000000001 * COIN;
    } else if (nHeight <= 90000) {
        nDevPayment = 600.000000001 * COIN;
    } else if (nHeight <= 100000) {
        nDevPayment = 300.000000001 * COIN;
    } else if (nHeight <= 110000) {
        nDevPayment = 150.000000001 * COIN;
    } else if (nHeight <= 120000) {
        nDevPayment = 75.000000001 * COIN;
    } else if (nHeight <= 130000) {
        nDevPayment = 35.000000001 * COIN;
    } else {
        nDevPayment = 250.000000001 * COIN;
    }
...
{
    int64_t nMNCollateral = 0;

    if (nHeight <= 20000) {
        nMNCollateral = 40000.00;
    } else if (nHeight <= 50000) {
        nMNCollateral = 400000.00;
    } else if (nHeight <= 80000) {
        nMNCollateral = 1000000.00;
    } else if (nHeight <= 90000) {
        nMNCollateral = 1000000.00;
    } else if (nHeight <= 100000) {
        nMNCollateral = 1000000.00;
...
{
    return (
        nAmountToCheck == (GetMNCollateral(nHeight) * COIN) || 
        nAmountToCheck == (GetMNCollateral(nHeight - floor(COLLATERAL_TRANSITION_BLOCKS / 2)) * COIN) ||
        nAmountToCheck == (GetMNCollateral(nHeight + floor(COLLATERAL_TRANSITION_BLOCKS / 2)) * COIN)
        );
}
...
{
    util::ThreadRename("encocoinplus-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    //Check that the block does not overmint nor undermit
    if (!IsBlockValueValid(block, nExpectedMint, nMint)) {
        return state.DoS(100, error("ConnectBlock() : incorrect block reward value (actual=%s vs expected=%s)",
    ...
    if (pindex->nHeight > last_pow_block) {
      ...
      if (nDevReward > 0) {
        CTxDestination destination = CBitcoinAddress(Params().GetConsensus().devAddress).Get();
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (nHeight > Params().GetConsensus().height_last_PoW && pindexPrev) { // pindexPrev is only null on the first block which is a version 1 block.
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> encocoinplusInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                encocoinplusInputs.push_back(stakeIn);
        }
        const bool hasEPGInputs = !encocoinplusInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasEPGInputs) {
                    ...
                    for (const CTxIn& encocoinplusIn : encocoinplusInputs)
                        if(encocoinplusIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasEPGInputs) {
                            for (const CTxIn& stakeIn : encocoinplusInputs)
            ...
            // Now that this loop if completed.
        }
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // encocoinplus: We use certain sporks during IBD, so check to see if they are
        // available. If not, ask the first peer connected for them.
        // TODO: Move this to an instant broadcast of the sporks.
        bool fMissingSporks = !pSporkDB->SporkExists(SPORK_5_MAX_VALUE);
