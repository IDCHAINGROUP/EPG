...
std::vector<CSporkDef> sporkDefs = {
    ...
    MAKE_SPORK_DEF(SPORK_5_MAX_VALUE,                       1000),          // 1000 EPG
    ...
    MAKE_SPORK_DEF(SPORK_90_BLOCK_REWARD_ENFORCEMENT,       4070908800ULL), // OFF
};
...
// encocoinplus: on startup load spork values from previous session if they exist in the sporkDB
...
void CSporkManager::ProcessSpork(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == NetMsgType::SPORK) {
        ...
        // encocoinplus: add to spork database.
