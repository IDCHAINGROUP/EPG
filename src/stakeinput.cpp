...
bool CEPGStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CEPGStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CEPGStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CEPGStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CEPGStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CBlockIndex* CEPGStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CEPGStake::ContextCheck(int nHeight, uint32_t nTime)
{
